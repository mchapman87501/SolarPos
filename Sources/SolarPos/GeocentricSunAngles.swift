...
public struct GeocentricSunAngles {
    ...
    public init(apparentSunLongitude lambda: Double,
        ...
        rightAscension = GeocentricSunAngles.getRA(
            apparentSunLongitude: lambda,
            meanObliquity: epsilon,
            geocentricLatitude: beta)
        declination = GeocentricSunAngles.getDecl(
            apparentSunLongitude: lambda,
            meanObliquity: epsilon,
            geocentricLatitude: beta)
    }

    private static func getRA(apparentSunLongitude lambda: Double,
                              meanObliquity epsilon: Double,
        ...
        let num1 = sin(rlambda) * cos(repsilon)
        let num2 = tan(rbeta) * sin(repsilon)
    ...
    private static func getDecl(apparentSunLongitude lambda: Double,
                                meanObliquity epsilon: Double,
                                geocentricLatitude beta: Double) -> Double {
        let rbeta = rads(beta)
        let repsilon = rads(epsilon)
