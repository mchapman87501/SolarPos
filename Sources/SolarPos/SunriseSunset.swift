...
public struct SunriseSunset {
    ...
    static func calcSunsetHourAngle(latitude: Double, declination: Double) -> Double
    {
        ...
        let d = rads(declination)
        let a = rads(-0.83) // altitude of center of solar disk

        let numer = sin(a) - sin(phi) * sin(d)
        let denom = cos(phi) * cos(d)
        ...
        -> Double 
    {
        ...
        let gmtMinutes = Double(ts.gmtOffset) * 60.0
