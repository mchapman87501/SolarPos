...
class AppendixA5Tests: XCTestCase {
    ...
    private func eq(
        ...
        let nonZero = abs(expected) >= 1.0e-6
        let fractError = (nonZero ? (abs((actual - expected) / expected)) : abs(actual - expected))
        let msg = msg ?? ""
        XCTAssertTrue(fractError <= epsilon, "\(msg): expected \(expected), actual \(actual), error \(fractError)")
    }
    ...
    private func assertArrayEq(actual: [Double], expected: [Double], msg: String) {
        ...
        let allPassed = closeEnough.allSatisfy { $0 }
